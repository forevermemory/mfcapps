...
)
{
	...
	else
	{
		...
		case OB_OPERATION_HANDLE_CREATE:
			...
			{
				DbgPrint("sys:>>>>>降权<<<<< curPID=%d 进程名=%s  目标进程PID =%d  目标进程名=%s \n",
...
void InstallProcessProtect()
{
	...
	//ob1 5项
	RtlInitUnicodeString(&ob1_callback_reg.Altitude, L"123456");
	...
	ob2_operation.Operations = OB_OPERATION_HANDLE_CREATE | OB_OPERATION_HANDLE_DUPLICATE;  // 只处理创建句柄的操作
...
//BOOLEAN IsMyCeProcess()
//{
//	PEPROCESS Process = PsGetCurrentProcess();
//
//	if (_strnicmp("12345678.exe", PsGetprocessImageFileName(Process), strlen("12345678.exe") == 0))
//	{
//		return TRUE;
//	}
//	return FALSE;
...
void AddPidToPrivilege(UINT32 pid)
{
	for (size_t i = 0; i < 256; i++)
	{
		if (提权的PID[i] == 0 || 提权的PID[i] == pid)
		{
			//是空位置
			提权的PID[i] = pid;
			DbgPrint("sys: 添加提权的PID =%d ", pid);
			break;
		}
	}
}

void RemovePidToPrivilege(UINT32 pid)
{
	for (size_t i = 0; i < 256; i++)
	{
		if (提权的PID[i] == pid) //相等表示找到了
		{
			提权的PID[i] = 0;
			DbgPrint("sys: 移除提权的PID =%d ", pid);
			//break;
		}
	}
	return;
...
BOOLEAN IsPidToPrivilege(UINT32 pid)
{
	if (pid == 0) return 0;
...
OB_PREOP_CALLBACK_STATUS my_privilege_callback(
	PVOID RegistrationContext,
	POB_PRE_OPERATION_INFORMATION OperationInformation
)
{
	...
	else
	{
		...
		case OB_OPERATION_HANDLE_CREATE:
			...
			// 提权
			//if (strcmp(目标进程名, 进程名) != 0 && strcmp(进程名, "12345678.exe") == 0)
			//{
			//	DbgPrint("sys: <<<<<提权>>>>>curPID=%d 进程名=%s  目标进程PID =%d  目标进程名=%s \n",
			//		curPID, 进程名, dwProcessId, 目标进程名);
			//	OperationInformation->Parameters->CreateHandleInformation.DesiredAccess = 0x1fffff;
			//}

			if (IsPidToPrivilege(curPID))
			{
				DbgPrint("sys: <<<<<提权>>>>>curPID=%d 进程名=%s  目标进程PID =%d  目标进程名=%s \n",
					curPID, 进程名, dwProcessId, 目标进程名);
				OperationInformation->Parameters->CreateHandleInformation.DesiredAccess = 0x1fffff;
...
HANDLE gs_HandleCallback_Privilege = NULL;//用来存放返回的句柄  以方便卸载对应功能
void InstallProcessPrivilege()
{
	
	OB_CALLBACK_REGISTRATION ob1_callback_reg = { 0 };
	OB_OPERATION_REGISTRATION ob2_operation = { 0 };
	//ob1 5项
	RtlInitUnicodeString(&ob1_callback_reg.Altitude, L"123"); // 越大越先执行,小的最后执行
	ob1_callback_reg.RegistrationContext = NULL;
	ob1_callback_reg.Version = OB_FLT_REGISTRATION_VERSION;// ObGetFilterVersion(); //OB_FLT_REGISTRATION_VERSION
	ob1_callback_reg.OperationRegistrationCount = 1;//只注册了一个回调pre
	ob1_callback_reg.OperationRegistration = &ob2_operation;
	...
	ob2_operation.Operations = OB_OPERATION_HANDLE_CREATE | OB_OPERATION_HANDLE_DUPLICATE;  // 只处理创建句柄的操作
	ob2_operation.PostOperation = NULL;
	ob2_operation.PreOperation = my_privilege_callback;
	ObRegisterCallbacks(&ob1_callback_reg, &gs_HandleCallback_Privilege); // /INTEGRITYCHECK 
	DbgPrint("sys:安装内存保护 gs_HandleCallback_Privilege=%p\n", gs_HandleCallback_Privilege);
}

void UnInstallProcessPrivilege()
{
	DbgPrint("sys:卸载内存保护,gs_HandleCallback_Privilege:%p \n", gs_HandleCallback_Privilege);
	if (gs_HandleCallback_Privilege)
	{
		ObUnRegisterCallbacks(gs_HandleCallback_Privilege);
	}
	DbgPrint("sys:卸载内存保护结束\n");
}
